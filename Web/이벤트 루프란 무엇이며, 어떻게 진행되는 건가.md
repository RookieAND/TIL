...
-   하나의 프레임에서 처리된다는 의미는 결국 하나의 호출에서 처리된다는 의미와 동일하다. 브라우저의 Paint의 경우에도 보통은하나의 프레임에서 처리된다.
...
결과적으로 상단의 코드에서 `baz` 는 10ms 보다 더 늦게 실행될 것이다. `setTimeout` 함수가 호출된 이후 실행된 `foo` 함수가 처리되기까지 오랜 시간이 걸린다면 콜 스택이 비워지지 않아 `baz` 함수가 실행될 수 없기 때문이다. 따라서 `setTimeout` 의 타이머가 항상 올바르게 작동한다는 보장이 없다.
...
3. 이후 브라우저의 렌더링이 필요한 요소가 있는지를 체크하고, 만약 그러하다면 렌더링을 진행한다.
    - resize, scroll 등의 이벤트를 실행하고, requestAnimationFrame에서 추가된 콜백을 실행한다.
...
- 또한 Micro Task 외의 어떤 Task (렌더링, Callback 등) 도 실행되지 않는다.

1. `perfoming a microtask checkpoint` 플래그가 false인지를 확인한다.
...
1. a 함수 호출, `console.log(a1)` 실행, 출력
2. b 함수 호출, `console.log(b1)` 실행, 출력 
3. c 함수 호출, `console.log(c1)` 출력
4. d 함수 호출, 첫 번째 `console.log(d1)` 실행, 출력
5. 두 번째  `console.log(d1)` 실행, 출력
6. .then 콜백은 백그라운드를 거쳐 마이크로 태스크 큐에 쌓임
7. d 함수 호출 완료 후 await를 만나고 async 함수 c는 중단된.
    async 함수의 나머지 Task 는 마이크로 태스크 큐에 쌓임
8. c 함수를 호출한 실행 컨텍스트 (b 함수) 로 돌아가서 `console.log(c2)` 실행, 출력
9. b 함수를 호출한 실행 컨텍스트 (a 함수) 로 돌아가서 `console.log(b2)` 실행, 출력
10. 콜스택이 모두 비워지고, 이벤트 루프가 마이크로 태스크 큐를 확인
       then 콜백, async 함수가 쌓여있음
11. .then 콜백 실행, `console.log(then!)` 출력.
12. async 함수 중단된 곳부터 이후로 실행,  `console.log(c2)` 출력 → c2
