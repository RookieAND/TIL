...
### 3. HTML Parsing이 다른 Parsing과 다른 점.
    ...
    - HTML 언어는 실수에 관대하다. 잘못된 태그 문법을 사용하더라도 오류를 내뿜지 않는다.
    - 설령 HTML이 잘못 작성되었더라도 브라우저에서는 이를 지적하려 하지 않는다.
    - HTML에서 `document.write` 와 같은 script 태그는 토큰을 추가할 수 있기 때문에, 입력 과정에서 파싱이 수정될 수 있다.
